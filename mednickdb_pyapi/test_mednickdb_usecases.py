...
def test_usecase_1():
    ...
    with open('testfiles/sleepfile.edf') as sleepfile:
        fid = med_api.upload_file(sleepfile, filename='sleepfile.edf', fileformat='sleep', study='PSTIM', version=1, subject=1, visit=1, session=1, filetype='raw sleep')
...
def test_usecase_2():
    ...
    #d)
    data = med_api.get_data(study='PSTIM', version=1)
    ...
    #e)
    data_raw_sleep = med_api.get_data(study='PSTIM', version=1, filetype='raw sleep')
...
def test_usecase_3():
    #a)
    med_api = MednickAPI('http://localhost:8001', 'test_ra_account@uci.edu', 'Pass1234')
    med_api.upload_data(data={'accuracy': 0.9}, study='PSTIM', version=1, subject=2, visit=1, session=1, filetype='MemTaskA')

    #b)
    correct_fids = [pytest.usecase_1_fid, pytest.usecase_2_fid]
    fids = med_api.get_file_ids(subject='PSTIM', version=1)
...
def test_usecase_4():
    ...
    with open('testfile/scorefile1.mat') as scorefile1:
        fid1 = med_api.upload_file(scorefile1, filename='sleepfile.edf', fileformat='scorefile', study='PSTIM', version=1,
                                  subject=1, visit=1, session=1, filetype='scorefile')

    with open('testfile/scorefile2.mat') as scorefile2:
        fid2 = med_api.upload_file(scorefile2, filename='sleepfile.edf', fileformat='scorefile', study='PSTIM', version=1,
                                  subject=1, visit=2, session=1, filetype='scorefile')
...
def test_usecase_5():
    #a)
    med_api = MednickAPI('http://localhost:8001', 'test_grad_account@uci.edu', 'Pass1234')
    data = med_api.search_datastore(query_string='study=PSTIM, accuracy>0.9')
